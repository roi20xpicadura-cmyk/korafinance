-- Kora v2 — schema + fundação (índice memory_active corrigido sem NOW())
...
-- Substitui o predicado parcial com NOW() (não-IMMUTABLE) por índice composto.
-- Filtro de validade fica no WHERE da query.
CREATE INDEX IF NOT EXISTS idx_kora_memory_user_expires
  ON public.kora_memory(user_id, expires_at);

ALTER TABLE public.kora_memory ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users read own memories" ON public.kora_memory;
CREATE POLICY "Users read own memories" ON public.kora_memory
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users delete own memories" ON public.kora_memory;
...
DROP POLICY IF EXISTS "Users read own interactions" ON public.kora_interactions;
CREATE POLICY "Users read own interactions" ON public.kora_interactions
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users delete own interactions" ON public.kora_interactions;
CREATE POLICY "Users delete own interactions" ON public.kora_interactions
  FOR DELETE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Admins read all interactions" ON public.kora_interactions;
...
CREATE TABLE IF NOT EXISTS public.kora_actions (
  ...
  action_type TEXT NOT NULL CHECK (action_type IN (
    ...
    'recategorize_transaction','create_budget','update_budget',
    'create_goal','update_goal','create_debt','update_debt',
    'create_coaching_plan','update_coaching_plan','add_memory'
  )),
...
DROP POLICY IF EXISTS "Users read own actions" ON public.kora_actions;
CREATE POLICY "Users read own actions" ON public.kora_actions
  FOR SELECT USING (auth.uid() = user_id);

-- 4. kora_coaching_plans
CREATE TABLE IF NOT EXISTS public.kora_coaching_plans (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  plan_type TEXT NOT NULL CHECK (plan_type IN (
    'debt_payoff','savings','budget_recovery','goal_achievement',
    'financial_education','couple_alignment'
  )),
  title TEXT NOT NULL,
...
DROP POLICY IF EXISTS "Users read own plans" ON public.kora_coaching_plans;
CREATE POLICY "Users read own plans" ON public.kora_coaching_plans
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users update own plan status" ON public.kora_coaching_plans;
...
DROP POLICY IF EXISTS "Users read own profile" ON public.kora_user_profile;
CREATE POLICY "Users read own profile" ON public.kora_user_profile
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Admins read all profiles" ON public.kora_user_profile;
...
DROP POLICY IF EXISTS "Users read own usage" ON public.kora_usage_limits;
CREATE POLICY "Users read own usage" ON public.kora_usage_limits
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Admins read all usage" ON public.kora_usage_limits;
...
BEGIN
  ...
  WHERE created_at >= CURRENT_DATE AND cost_usd IS NOT NULL;
  RETURN total;
END;
$$;

-- 11. delete_user_kora_data (LGPD)
CREATE OR REPLACE FUNCTION public.delete_user_kora_data(p_user_id UUID)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE caller UUID := auth.uid(); counts JSONB;
BEGIN
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    trait_planner=0.5, trait_frugal=0.5, trait_risk_tolerant=0.5,
    trait_emotional_spender=0.5, trait_social_oriented=0.5,
    prefers_direct_tone=TRUE, prefers_data_heavy=FALSE, prefers_emotional_support=FALSE,
    life_context='{}'::jsonb, spending_patterns='{}'::jsonb,
    profile_confidence=0.1, interactions_analyzed=0, last_updated_at=NOW();
  RETURN counts;
END;
$$;

-- 12. handle_new_user estendido
